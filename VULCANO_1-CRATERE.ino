...
int CIRC_LEDS = 144*2.5-10;
int COLATA_LEDS = NUM_LEDS - CIRC_LEDS;
...
void loop() {
  ...
  //POT 2
  attesa = (unsigned long)map( analogRead(1),1024,0,0,5000 );
  ...
  if ((colata < COLATA_LEDS)) {

    if (((millis() - ttc) > attesa/10) && flag_start) {
      ttc = millis();
      colata++;
    }
    
    for (uint16_t i=colata+CIRC_LEDS; i<NUM_LEDS; i++) {
      leds[i].red = 0;
      leds[i].green = 0;
      leds[i].blue = 0;
    }
